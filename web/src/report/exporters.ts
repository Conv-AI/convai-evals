...
function csvRow(r: PerRowResult): string[] {
  ...
  return [
    ...
    r.input_kind,
    esc(r.resolved_expectation?.run_llm ?? ""),
    r.received_state == null ? "" : String(r.received_state.bot_busy),
    r.received_state == null ? "" : String(r.received_state.user_speaking),
    esc(r.resolved_expectation?.category ?? ""),
    esc(r.resolved_expectation?.resolution ?? ""),
    r.expected.behavior,
